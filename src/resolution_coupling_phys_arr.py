...
def resolve_phys_arr(d_Nt, d_Nr, d_P, d_SNR_low, d_SNR_high, d_low_ang, d_high_ang, d_N, d_beta , d_num_trials): 
    # QPSK generator
    def s_qpsk(d_P):
    ...
    v_theta_deg = np.array([d_low_ang, d_high_ang]).reshape(1, -1)
    ...
    for d_SNR in tqdm(v_SNR_range, desc="Processing SNRs"):
        ...
        for _ in range(d_num_trials):
            ...
            m_s = np.vstack([s_qpsk(d_P) for _ in range(d_Nt)])
            m_y = m_C @ m_A @ m_s
            ...
            # Eigen decomposition for MUSIC
            v_eigvals, m_eigvecs = np.linalg.eig(m_R)
            v_eig_order = np.argsort(np.abs(v_eigvals))
            m_V = m_eigvecs[:, v_eig_order[:d_Nr - d_Nt]]  # Noise subspace
            ...
            v_top_angles = get_top_angles(find_peaks(v_results_MUSIC, height=-100)[0], v_results_MUSIC, d_Nt, v_theta_g)
...
# ===== Example =====
d_Nt = 2            # Number of sources        
d_Nr = 5            # Number of antennas
d_P = 100           # Number of snapshots 
d_SNR_low = 10      # lower bound of tested SNR
d_SNR_high = 20     # upper bound of tested SNR 
d_low_ang = 10      # angle 1 in degrees
d_high_ang = 20     # angle 2 in degrees
d_N = 4             # Banded of coupling coefficient
d_beta = 0.01       # Largest agnitude of coefficient
d_num_trials = 100  # Number of Monte Carlo runs

v_SNR_range, v_ratios_MUSIC = resolve_phys_arr(d_Nt, d_Nr, d_P, d_SNR_low, d_SNR_high, d_low_ang, d_high_ang, d_N, d_beta , d_num_trials)
